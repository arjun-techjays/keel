...
Print a categorised snapshot of the current open set so the lead sees the whole landscape at a glance. **Group by discipline** (Scope Hygiene, Product, UX, Engineering, Data/ML, Security, QA, Ops, Change, Commercial, Delivery, RAID, and **Project-specific (`PRJ-*`)** for lens-derived items, Law 15), and within each list one line per open question: `ID · [BLOCK?] · tag · origin · the question` — surface the `raised-by: keel-review` origin so the lead can see which items came from the adversarial pass (these often ship with a suggested disposition ready to accept). Lead with a header line of totals:
...
**5c · Project-specific items (`PRJ-*`, Law 15) close exactly like catalog dimensions.** When the open item concerns a `PRJ-*` dimension the lenses derived — a record-relationship rule, a liability boundary, a failure tail, a handoff-inaction path — drive it to a real sourced answer or a disposition just like any catalog Gap. The **`PRJ-NN` id goes into the decision-log entry verbatim**, so `keel-map`'s re-score updates the right row in `.keel/project-dimensions.md` and the coverage map. A `PRJ-*` gets no lighter treatment for living in engagement state: Partial blocks like a Gap, Skip leaves it red, and a disposition note still has to carry its grounds (Part B).
