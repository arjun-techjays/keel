...
- **Uncovered decision area — the gap-hunt (Law 15).** Step back from the sections and ask the question no per-section probe asks: *is there a decision area this pack settles nothing about, that no dimension — catalog or `PRJ-*` — even names?* Walk the **seven discovery lenses** (constitution Part E) as a checklist against the rendered pack — a data object whose relationships / free-text semantics / merge-against-target are undecided (L1); a business outcome with no liability owner under a fixed bid (L2); a failure tail or unhandled-input default left blank (L3); a handoff whose non-action path is silent (L4); an integration with no data-correctness owner (L5); a peak / cutover window unquantified (L6); a named domain rule never resolved (L7). A genuinely uncovered area is a finding, represented as `PRJ-GEN-<n>` (below). This is the **second net**: `map`'s lenses run *before* scoring; this one runs adversarially against the *finished* pack, catching what the first pass missed — because the standard's own completeness is bounded by what the lenses surface, the review must try, independently, to find what they didn't.
...
- **a gap-hunt (uncovered-area) finding is a `PRJ-GEN-<n>`:** it cites no catalog dimension, so its ledger **Dimensions cell carries the bare token `PRJ-GEN`** (never `PRJ-GEN-<n>` — a digit suffix there mis-parses into a phantom id), **Tag = BLOCK only when it gates acceptance / price / scope**, else `—`; Disposition = OPEN; provenance `raised-by: keel-review; PRJ-GEN-<n>`. If `clarify` and the next `keel-map` confirm the area is real and recurring, `map` **promotes** it to a scored `PRJ-NN` dimension in `.keel/project-dimensions.md`; otherwise it stays a tracked question. Either way it is never lost — `map`'s preserve-rule carries `raised-by: keel-review` rows through re-score.
...
- **Low** — wording.
- **Gap-hunt (uncovered-area, Law 15)** — an area no dimension (catalog or `PRJ-*`) covers, found by the discovery-lens lens above. It is **High → 🔴 FREEZE-BLOCKED only when it gates acceptance, price, or scope** (e.g. an undecided liability boundary under a fixed bid, a failure tail that turns into unpriced work); otherwise it is a **tracked open question (Medium/Low), non-blocking** — surfaced for the lead, not gating. This keeps the second net from flooding the freeze gate with speculative areas while still stopping the material ones.
