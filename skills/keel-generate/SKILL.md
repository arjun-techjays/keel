...
- **structure:** each document contains every section from its Part F table, in order (and `2-scope.md` has the `F2.3–F2.11` block once per module) — no Part F section dropped silently (the present-if-used exception: a synthesis-only section with no Renders-from — `F2.24` — appears when a `PRJ-*` routes there and is omitted-with-reason otherwise);
- **coverage:** every Covered dimension is rendered into ≥1 section that lists its ID (invert the crosswalk) — nothing covered-but-unwritten; a Covered `PRJ-*` dimension renders into its `.keel/project-dimensions.md` 'Renders into' section, or `F2.24`;
