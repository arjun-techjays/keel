...
export function ConnectAgent({
  ...
  // people who already have a token can reveal the command with a placeholder
  const [reveal, setReveal] = useState(existingTokenCount > 0);
  const [copied, setCopied] = useState(false);

  const generated = !!state?.token;
  const token = state?.token ?? "<YOUR_TOKEN>";
  const showCommand = generated || reveal;
    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6" style={{ backgroundColor: "rgba(20,22,26,0.4)" }}>
          <div className="flex w-full max-w-[560px] flex-col gap-5 rounded-[16px] border border-hairline bg-white p-7 shadow-xl">
            ...
            {/* Step 1: get a token (hidden once one is generated this session) */}
            {!generated && (
              <div className="flex flex-col gap-3 rounded-[12px] border border-hairline bg-panel p-4">
                <span className="text-[12px] font-semibold text-ink">
                  {existingTokenCount > 0 ? "Use a token" : "Step 1 — generate a token"}
                </span>
                <form action={formAction} className="flex items-center gap-2">
                  <input
                    name="name"
                    placeholder="Token name (e.g. my laptop)"
                    className="flex-1 rounded-[9px] border border-hairline bg-white px-3 py-2 text-[13px] text-ink outline-none focus:border-cobalt"
                  />
                  <button type="submit" disabled={pending} className="flex shrink-0 items-center gap-1.5 rounded-[9px] bg-cobalt px-3.5 py-2 text-[13px] font-semibold text-white disabled:opacity-60">
                    <KeyRound className="h-4 w-4" strokeWidth={2} />
                    {pending ? "Generating…" : "Generate token"}
                  </button>
                </form>
                {state?.error && <p className="text-[12px] font-medium text-gap">{state.error}</p>}
                {existingTokenCount > 0 && !reveal && (
                  <button onClick={() => setReveal(true)} className="self-start text-[12px] font-semibold text-cobalt">
            ...
            {/* Step 2: the command (only once a token exists or the user has one) */}
            {showCommand && (
              <div className="flex flex-col gap-3">
                {generated ? (
                  <div className="flex items-center gap-1.5 text-[12px] font-semibold text-covered">
                    <Check className="h-4 w-4" strokeWidth={2.5} /> Token created — copy the command now, it won&apos;t be shown again.
                  </div>
                ) : (
                  <span className="text-[12px] font-medium text-muted-ink">
                    Run this, replacing <code className="font-mono text-ink">&lt;YOUR_TOKEN&gt;</code> with your saved token:
                  </span>
                )}
            ...
            <div className="flex items-center justify-between border-t border-hairline pt-4">
              <Link href="/settings" className="text-[13px] font-medium text-muted-ink hover:text-ink">
                Manage tokens
              </Link>
